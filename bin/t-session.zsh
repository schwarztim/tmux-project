# t-session.zsh — Project-first tmux session manager (zsh version)
# Source this from ~/.zshrc:  source /path/to/tmux-project/bin/t-session.zsh
...
typeset -gA _TP_ALIASES=()
typeset -gA _TP_FAVORITES=()
typeset -gA _TP_PROJECT_ROOT_LABELS=()
typeset -gA _TP_PROJECT_ROOT_BASES=()
...
_tp_unique_project_name() {
    ...
    while (( ${+_TP_PROJECTS[$candidate]} )) && [[ "${_TP_PROJECTS[$candidate]}" != "$project_path" ]]; do
...
_tp_add_project() {
    ...
    [[ -z "$raw_name" || -z "$project_path" ]] && return

    local display="${_TP_ALIASES[$raw_name]:-$raw_name}"
    local candidate="$display"

    if [[ -n "$root_label" && -n "${_TP_PROJECT_ROOT_BASES[$display]:-}" ]] && (( ! ${+_TP_PROJECTS[$candidate]} )); then
        candidate="${root_label}/${display}"
    fi

    if (( ${+_TP_PROJECTS[$candidate]} )); then
        [[ "${_TP_PROJECTS[$candidate]}" == "$project_path" ]] && return
...
_tp_load_root_projects() {
    [[ -f "$TMUX_PROJECT_ROOTS" ]] || return

    local entry root_label root_path project_dir name
    while IFS= read -r entry; do
        ...
        for project_dir in "$root_path"/*(N/); do
            name="${project_dir:t}"
            [[ -z "$name" || "$name" == .* ]] && continue
...
_tp_load_favorites() {
    _TP_FAVORITES=()
    [[ -f "$TMUX_PROJECT_FAVORITES" ]] || return
...
_tp_toggle_favorite() {
    ...
    if _tp_is_favorite "$project_name"; then
        for favorite in "${(@ko)_TP_FAVORITES}"; do
...
_tp_in_tmux_picker() {
    ...
    local _sesh="${commands[sesh]:-$(command -v sesh 2>/dev/null)}"
    if [[ -n "$_sesh" ]]; then
...
t() {
    local _fzf="${commands[fzf]:-$(command -v fzf 2>/dev/null)}"
    local _tmux="${commands[tmux]:-$(command -v tmux 2>/dev/null)}"
    ...
    for proj in "${(@ko)_TP_PROJECTS}"; do
        local count=0
    ...
    picker_output=$(printf "%s\n%b" "+ New Project" "$project_lines" | "$_fzf" --no-sort --ansi \
    ...
    if [[ "$proj_choice" == "+ New Project" ]]; then
        ...
        local suggested_path="${parent_dir}/$new_name"
        printf "Project path [%s]: " "$suggested_path"
        local new_path
        read new_path
        new_path="${new_path:-$suggested_path}"
        [[ -z "$new_path" ]] && return
        ...
        printf "Session label (creates ${new_name}/<label>): "
        local label
        read label
