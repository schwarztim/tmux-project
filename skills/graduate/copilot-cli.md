---
name: graduate
description: "Graduate the current conversation into a project. Synthesizes conversation context into .github/copilot-instructions.md and .claude/context.md at the target project path, registers in T (tmux-project) if new, copies a kickoff prompt to clipboard, and opens a tmux session in the project directory."
---

# Graduate Conversation to Project

You are helping the user transfer the current conversation's context into a project directory so they can pick it up in a new session scoped to that project.
...
**4b.** Write the synthesized context to BOTH files:
...
Write the same content to both files. Use your file writing tool to create each one.
...
- Context written to `<PROJECT_PATH>/.github/copilot-instructions.md` and `<PROJECT_PATH>/.claude/context.md`
